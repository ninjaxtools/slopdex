...
| `src/code-index.ts` | Public index facade and collaborator lifecycle. |
| `src/indexing/` | File preparation, Git/working-tree reconciliation, embeddings, and source selection. |
...
| `src/embeddings/`, `src/descriptions/`, `src/rerankers/` | Provider requests, registries, and provider profiles. |
| `src/storage/` | SQLite schema, durable artifact caches, transactions, metadata, and vector queries. |
| `src/search/` | Analysis scoring selection and cross-index neighbor discovery. |
| `src/analysis/` | Cohesion location, gap scores, aggregate affinity, and groups. |
...
`CodeIndex.describe` reuses the natural-language search, preserves reranker order and scores, groups results by file, keeps the highest result similarity per file, and collects file descriptions, callable descriptions/sources/locations, and relevance scores into a `DescribeContext`. Complete indexed sources are attached only for files strictly above the caller's full-file threshold. Working-tree sources must remain regular files with their indexed content hash; one validation or read failure aborts every whole-file source and is reported through the context's `fileContentErrors` so the caller can still proceed with descriptions and callable code. `OpenAIDescriptionProvider.describeContext` turns that context into a one-shot discovery explanation using dedicated instructions that forbid implementation advice.
