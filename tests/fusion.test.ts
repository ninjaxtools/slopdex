...
import type { CrossSearchOptions, CrossSearchResult, DescriptionProvider, EmbeddingProvider } from "../src/types.js";
import { temporaryRoot, write } from "./helpers.js";

// Balanced is not the best code or purpose match, but is the best combined match.
const vectors: Record<string, { code: number[]; description: number[] }> = {
  anchor: { code: [1, 0], description: [1, 0] },
  implementation: { code: [1, 0], description: [0, 1] },
  purpose: { code: [0, 1], description: [1, 0] },
  balanced: { code: [0.8, 0.6], description: [0.8, 0.6] },
};
...
async function makeIndex(names: string[], descriptionModel: string | null = "purpose-model"): Promise<CodeIndex> {
  ...
  const index = new CodeIndex({ rootDir: root, provider, descriptionProvider });
  onTestFinished(() => index.close());
  await index.updateFiles({ upsert: paths });
  if (descriptionModel !== null) await index.useDescriptions();
  return index;
}
...
function removeDescription(index: CodeIndex, name: string): void {
  const db = new DatabaseSync(index.indexPath);
  db.prepare("UPDATE functions SET description = NULL, description_embedding_id = NULL WHERE name = ?").run(name);
  db.close();
}
...
describe("combined code and purpose analysis", () => {
  ...
  it.each([[false, false], [true, false], [false, true]])(
    "uses code-only scoring without complete indexes (source=%s, target=%s)", async (sourceDescriptions, targetDescriptions) => {
      const source = await makeIndex(["anchor"], sourceDescriptions ? "source-model" : null);
      const target = await makeIndex(["implementation", "purpose", "balanced"], targetDescriptions ? "target-model" : null);
      ...
      expect(result!.matches[0]).not.toHaveProperty("descriptionSimilarity");
      expect(result!.scoring).toMatchObject({ similarityMode: "code", similarityWeights: { code: 1, description: 0 } });
    },
  );

  it.each(["source", "target"])("falls back for the entire cross-index analysis if %s descriptions are incomplete", async (side) => {
    const source = await makeIndex(["anchor", "purpose"]);
    const target = await makeIndex(["implementation", "balanced"]);
    removeDescription(side === "source" ? source : target, side === "source" ? "purpose" : "balanced");
  ...
  it("uses fused neighbors for reciprocity, gaps, file metrics, and groups", async () => {
    ...
    expect(report.metrics.weightedMeanDistance).toBeCloseTo(3);
  ...
  it.each([false, true])("falls back for same-index cross-search and cohesion (incomplete=%s)", async (incomplete) => {
    const index = await makeIndex(Object.keys(vectors), incomplete ? "description-model" : null);
    if (incomplete) removeDescription(index, "purpose");
    ...
    expect(report.pairs[0]).not.toHaveProperty("descriptionSimilarity");
    expect(report.metrics.sameFolderRatio).toBe(1);
  });
  ...
  it("preserves candidate filters, self-exclusion, and symmetric deduplication", async () => {
    ...
    const filtered = index.similarToFunction(anchor.id, {
      includeDescriptions: true, limit: 1, minSimilarity: 0,
